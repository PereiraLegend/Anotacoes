...
def criarArquivo():
    ...
    def inserindoDados():
        def aprentarDados():
            ...
            tk.Button(janelaapresentar, text="Voltar o menu", command=janelamenu).grid(row=13, column=1)
...
def abrirArquivo():
    ...
    NomeArquivo = tk.Entry(janelaabrir, width=10).grid(row=1, column=0)
    tk.Button(janelaabrir, text="Abrir").grid(row=2, column=0)
...
def editarArquivo():
    ...
    tk.Button(janelaeditar, text="Voltar o menu", command=janelaeditar.destroy).grid(row=0, column=0)

def deletarArquivo():
    janeladeletar = tk.Toplevel()
    janeladeletar.title("Deletar Arquivo")
    janeladeletar.geometry('300x270+800+300')
    tk.Button(janeladeletar, text="Voltar o menu", command=janeladeletar.destroy).grid(row=0, column=0)
